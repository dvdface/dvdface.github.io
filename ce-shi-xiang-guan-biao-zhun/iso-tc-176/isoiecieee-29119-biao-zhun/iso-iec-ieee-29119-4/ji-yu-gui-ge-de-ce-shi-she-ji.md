...
当被测对象有很多参数\(每个参数有很多离散取值\)，参数和参数之间又需要进行交互，覆盖它们需要大量的测试用例；使用组合测试设计技术，可以有效减少用例数量；

被测对象参数较多时，采用全组合的方式进行覆盖的话，会导致生成大量的用例，因此常会采取N-wise的方式，即仅覆盖N个参数的所有取值组合，一般这个N和被测对象里面if-else的深度有关，越深越需要更大的N去覆盖，一般不超过6可以检出大部分缺陷；
