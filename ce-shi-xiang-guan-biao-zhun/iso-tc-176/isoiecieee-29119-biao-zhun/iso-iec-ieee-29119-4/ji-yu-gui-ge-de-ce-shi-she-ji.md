...
等价划分实际上暗含了一个假设，即，假定系统对划分的处理是类似的；如果系统不满足这个假设，则会导致风险；因此，做等价划分时，最好分析实际的数据类型，按照数据类型进行划分比较稳妥；如，VLAN 标记业务上行数据可以分为带VLAN和不带VLAN两种，也可以分为不同的二层协议类型，以及对各协议再做控制面与数据面划分；后者，更贴近实际使用的划分，测试会更加稳妥；
