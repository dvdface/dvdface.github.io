...
这个软件质量模型描述了八种质量特性，即，定义了八种作为被测对象的质量属性；

这八种质量特性请参考子章节：ISO25010质量模型；

测试要度量被测对象中较重要的质量特性；

为了测试这些质量特性，就需要发起相应的子测试流程；

当制定测试计划的时候，测试经理需要考虑所有的质量特性；把测试的重点放在质量特性的差异上；如，银行系统强调安全、安全相关的应用强调稳定；

### 测试依据

所谓测试依据，就是进行测试计划、设计、参考、实现和管理的依据；
...
等等。在软件开发生命周期的不同阶段，测试依据各不相同；

以下是一些测试依据来源的示例：

* 对文档格式或内容的预期，一般是以规范和Checklist表提出的；
* 客户、用户对软件系统的预期，主要是以需求规格的形式提出的，在文档里以应该类的陈述句、用例或用户故事的形式出现。可能是形式化、也可能是非形式化的；有时候还会包含一些法律法规相关的文档；
* 测试人员或领域专家基于对现有系统的经验对用户期望的预判；
* 系统组件之间的直接或间接接口的要求。可能是形式化的、也可能是非形式化的。一般从架构设计文档中的设计图中提取，或者从形式化描述的协议中提取；
* 软件详细设计中的要求；
...
回归测试则是在复测的基础上更进一步，复测只是为了验证问题得到了修复，而回归测试还要验证修复没有引发新的问题；

在做测试计划的时候，一定要考虑测试完成环节中需要进行复测和回归测试；

### 测试设计技术

测试设计技术存在两种：静态和动态；

测试设计技术是为了帮助测试人员更加有效的找出被测对象中的缺陷；

静态测试主要是发现文档或源码中的明显缺陷；

动态测试主要是通过执行被测对象来查找缺陷；

#### 静态测试设计技术

静态测试技术有很多种，如，静态代码扫描、跨文件的可追溯性分析、通过评审查找问题、提供软件产品的其他信息等等；

静态测试是测试的一种形式，常常作为测试策略的一部分；

评审\(Review\)的主要目的就是查找缺陷，次要目的是彼此之间相互沟通和了解，达成一致；

常见的评审技术在IEEE1028:2008中有定义，如下图所示：

![&#x8BC4;&#x5BA1;&#x6280;&#x672F;&#x4E4B;&#x95F4;&#x7684;&#x533A;&#x522B;\(1\)](../../../../.gitbook/assets/image%20%2832%29.png)

![&#x8BC4;&#x5BA1;&#x6280;&#x672F;&#x4E4B;&#x95F4;&#x7684;&#x533A;&#x522B;\(2\)](../../../../.gitbook/assets/image%20%2846%29.png)

审查\(Inspection\)是评审技术的一种，一样也可以预防缺陷；
...
根据如何分析输入参数对动态测试设计方法分类，主要分为以下三类：

* 基于规格的测试设计：测试用例的输入和输出均来自测试依据；
  * 选择哪种基于规格的测试设计技术，取决于测试依据和被测对象所涉及的风险；
* 基于结构的测试设计：测试用例是分析被测对象的结构特征而来，如代码结构、菜单结构；
  * 选择哪种基于结构的测试设计技术，取决于测试依据和被测对象所涉及的风险；
* 基于经验的测试设计：根据测试人员的经验而来，如错误猜测法、探索性软件测试法等等；

## 测试实践

现实生活中，存在很多测试计划和实现测试的实践方法；

### 基于需求的测试

采取基于需求的测试实践，主要是为了确保被测对象的需求均已实现，满足用户需求；

在测试计划、设计、实现和执行阶段与相关方沟通的时候，均以需求为单元\(我要验证哪些需求，哪些需求通过了验证等等）进行沟通；

基于需求的测试主要使用用例进行测试（即，写用例，而不是不写）；用例会在设计阶段进行设计与编写，在执行阶段进行执行；测试用例会根据在测试过程中定义好的优先级顺序执行；对测试执行结果的分析，会更进一步的完善测试设计与用例；

基于需求的测试可以辅以其他的测试实践，一起来对需求进行充分的验证；如用探索性软件测试来覆盖一些基于需求测试覆盖不到的风险；

在实施基于需求的测试时，主要面临两个问题；首要的问题是，需要有清晰的需求作为输入，如果需求含混不清，那么测试就难以进行；第二个问题是，受制于预算的限制，可能无法对需求进行全面验证；通常，基于需求测试的人员会为需求补充一些基本信息，如优先级信息，对比较重要的需求尽早地更彻底的验证；

### 基于模型的测试

基于模型的测试就是用模型对被测对象的行为进行建模，然后把这个模型作为测试的依据；
...
所有测试均使用模型来表示被测对象的预期行为，且把这个模型用作测试的依据；

模型可能是以下方式呈现：

* 使用自然语言对需求进行描述，如测试规格就以IPO的方式描述；
* 以图形的方式呈现，如状态机、STRIDE模型；
* 以表格的方式呈现，如决策表；
* 以数学公式的方式呈现；

无论是基于规格的测试设计，还是基于结构化的测试设计，均可以使用模型分析得到输入和预期输出；

基于模型的测试实践和其他实践截然不同，但是它仍然是基于模型预期行为的；基于模型的测试，因为非常形式化和具体，所以可以从模型中获得大量的有用的测试信息；一般采用MBT的测试方法，还能通过自动化生成测试脚本来提升自动化水平；

一般使用MBT测试时，一个要求是一旦出错容易造成巨大的损失，另外一个要求是要便于建模；

建模时，一般使用UML图形记号；使用MBT，一般只用维护模型，通过模型生成脚本，而无需直接维护脚本，节省工作量；
